...
const APP_ID = 'amzn1.ask.skill.b439de85-006d-4dc1-80ef-b977b90fde3d'; // jshint ignore:line
...
const handlers = {
    ...
    'GetFirstEventIntent': function () {
        const categorySlotValue = this.event.request.intent.slots && this.event.request.intent.slots.day.value;
...
function selectCurrentDeals(deals, startIndex) {
    let speechOutputContent = '';
    let cardOutputContent = '';
    for (let i = startIndex; i < Math.min(deals.length, startIndex + PAGINATION_SIZE); i++) {
        speechOutputContent += '<p>' + deals[i] + '</p>';
        cardOutputContent += deals[i] + '\n';
    }
    return {
